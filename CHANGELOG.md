...
- Multi-workspace UI — шесть режимов работы: Аналитика, Макеты, Библиотека, React Preview, Обсуждение, Передача в разработку
- WorkspaceBar — вертикальная панель навигации по режимам (48 px, иконки Lucide, тултипы)
- AnalyticsView — сессии и чат с AI, генерация PRD-документа по результатам диалога
- LibraryWorkspaceView — дерево токенов (цвета, типографика, эффекты, компоненты, иконки), редактор токена, публикация библиотеки
- DiscussionView — канвас с пронумерованными пинами комментариев, треды, разрешение, фильтрация по статусу / странице / автору
- HandoffView — трёхшаговый процесс передачи разработчикам: генерация документации, ревью, шеринг ссылки
- ReactPreviewView — предпросмотр в iframe с preset-устройствами (Mobile 390 / Tablet 768 / Desktop 1280 / Fluid), лог взаимодействий
- AI-чат с LM Studio — локальные модели, сессии, поддержка ACP (Claude Code) через SSE-транспорт

### Fixes

- Vue 3.5 реактивный деструктуринг пропсов — исправлены ошибки в `ToolbarRoot` и остальных renderless-компонентах
- Подавлены предупреждения «Extraneous non-props attributes» в renderless-компонентах через `inheritAttrs: false`

## 0.11.6 — 2026-04-08

### Fixes

- Switch `@norka/core` build from `tsgo` + `fix-esm-import-path` to `tsdown` — fixes bare directory imports that broke Node.js and Bun consumers
...
- Fix `@norka/core` published package containing stale import paths from before the domain module restructuring — CLI and MCP installs from npm now resolve correctly
...
- Fix `@norka/mcp` release packaging so the published npm tarball includes its built `dist/` CLI and server entrypoints deterministically
- Fix `@norka/core` release build configuration so CI publish jobs include Node and Bun ambient types when compiling package artifacts
...
- Attempt to fix `@norka/mcp` npm package contents so the published CLI includes its built `dist/` entrypoints
- Fix `@norka/vue` npm package metadata and build output so the published package resolves from `dist/` while local workspace development keeps using source aliases
...
- npm release pipeline now publishes `@norka/core`, `@norka/cli`, `@norka/mcp`, and `@norka/vue` together on version tags
...
- New CLI commands: `norka convert` for document conversion, `norka formats` to inspect readable/writable/exportable formats, and `norka lint` for design consistency, structure, and accessibility checks
...
- Fix headless CLI font fallback — bundled Inter font now ships with `@norka/core` and loads without a web server
...
- XPath query command — `norka query design.fig "//FRAME[@width < 300]"` to find nodes by type, attributes, and tree structure using XPath selectors
- CSS Grid layout mode — select a frame, click the grid icon in the auto layout toolbar to switch from flex to grid. Configure column/row tracks (fr, fixed px, auto), column and row gaps, and per-side padding. Powered by a [Yoga fork](https://github.com/norka/yoga/tree/grid) with cherry-picked CSS Grid PRs from upstream
...
- Tailwind CSS v4 JSX export — export selections as HTML with Tailwind utility classes (`<div className="flex gap-4 p-3">`) from the Code panel, CLI (`bun norka export --format jsx --style tailwind`), or programmatically via `sceneNodeToJSX(id, graph, 'tailwind')`. Supports layout, sizing, colors, border radius, opacity, rotation, overflow, shadows, blur, and typography. Uses v4 spacing semantics (px/4 multiplier) with automatic fallback to arbitrary values.
- Code panel format toggle — switch between Norka (custom components) and Tailwind (HTML + utility classes) output
- Homebrew tap — `brew install norka/tap/norka` for macOS (arm64 + x64), auto-updated on each release
...
- CLI-to-app RPC bridge — all CLI commands work against the running app when no file is specified. Start the app, then run `bun norka tree` to inspect the live document
- VitePress docs site — user guide, reference, architecture, and development docs at norka.dev with 6 locales (en, de, fr, es, it, pl), SEO (OG tags, hreflang, JSON-LD, sitemap), and dark theme
...
- Fix broken test imports — stale `../../src/engine/` paths updated to `@norka/core`
...
- SVG export — export selections as SVG from the export panel, context menu, CLI (`bun norka export --format svg`), or MCP/AI tools (`export_svg`). Supports rectangles, ellipses, lines, stars, polygons, vectors, text with style runs, gradients, image fills, effects, blend modes, clip paths, and nested groups (#46)
...
- Fix `@norka/core` dep in MCP package: `workspace:*` for local dev (pnpm resolves at publish time)
...
- MCP server (`@norka/mcp`) — 29 tools for headless .fig editing via stdio (Claude Code, Cursor, Windsurf) or HTTP (Hono + Streamable HTTP with sessions)
- `norka-mcp` and `norka-mcp-http` binaries — install globally via `bun add -g @norka/mcp`

### Build

- All packages emit JS via tsgo + fix-esm-import-path — `@norka/core` and `@norka/mcp` work on Node.js without Bun
- Core package exports: `bun` condition → src (dev), `import` condition → dist (npm consumers)
- `@norka/mcp` added to CI publish workflow
...
- Type `window.__NORKA_STORE__` globally, remove ad-hoc casts from tests
...
- Figma clipboard: copy/paste between Norka and Figma
...
- Runs at [app.norka.dev](https://app.norka.dev)
- No installation required
- File System Access API for save/open (Chrome/Edge), download fallback elsewhere

### Documentation

- [norka.dev](https://norka.dev) — VitePress site with user guide, reference, and development docs
