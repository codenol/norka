...
2. В Norka использовать **preview-only подключение**:
   - Глобально в `src/main.ts`: только `primereact/resources/primereact.min.css` и `primeicons/primeicons.css`.
   - DS-стили подключать только в зоне JSON preview (`.ds-preview-root`) через `src/design-system/preview-runtime.scss`.
   - Не подключать `primeflex` и full DS overrides глобально: они конфликтуют с Tailwind shell.

3. Настроить алиасы в `tsconfig.json` и `vite.config.ts`:
...
- JSON renderer runtime-адаптеры находятся в `src/design-system/runtime`:
