...
> extends Component<FieldProps<T, S, F>, ObjectFieldState> {
     ...
     */
    onPropertyChange =
        (name: string, addedByAdditionalProperties = false) =>
        (value: T | undefined, newErrorSchema?: ErrorSchema<T>, id?: string) => {
     ...
     */
    onDropPropertyClick = (key: string) => (event: DragEvent) => {
    ...
    /** Computes the next available key name from the
     * `preferredKey`, indexing through the already existing keys until one
     ...
     */
    onKeyChange = (oldValue: any) => (value: any, newErrorSchema: ErrorSchema<T>) => {
        if (oldValue === value) {
            return;
        }
        const { formData, onChange, errorSchema } = this.props;
    ...
    /** Handles the adding of a new
     * additional property on the given `schema`. Calls the `onChange` callback once the new
    ...
    handleAddClick = (schema: any) => () => {
        ...
        let type: RJSFSchema['type'];
        if (isObject(schema.additionalProperties)) {
            type = schema.additionalProperties.type;
            let apSchema: any = schema.additionalProperties;
    ...
    render() {
        ...
        const templateProps = {
            ...
            description,
            value: orderedProperties.map((key) => {
                const addedByAdditionalProperties = has(schema, [PROPERTIES_KEY, key, ADDITIONAL_PROPERTY_FLAG]);
                const fieldUiSchema = addedByAdditionalProperties ? uiSchema.additionalProperties : uiSchema[key];
                const hidden = getUiOptions<T, S, F>(fieldUiSchema).widget === 'hidden';
                const fieldIdSchema: IdSchema<T> = get(idSchema, [key], {});
                const rawHelp = fieldUiSchema?.['ui:help'];
                ...
                return {
                    ...
                    value: (
                        <SchemaField
                            ...
                            onChange={this.onPropertyChange(key, addedByAdditionalProperties)}
                            onBlur={onBlur}
                            onFocus={onFocus}
                            registry={registry}
                            disabled={disabled}
        ...
        const dataTable = templateProps.value.map((item) => ({
            param: item.value.props.name,
            props: {
                name: item.value.props.name,
                required: item.value.props.required,
                rawHelp: item.value.props.uiSchema['ui:help'],
            },
            value: item.value,
        }));
        ...
        return (
            <DataTableDynamic
                ...
                header={uiSchema['ui:options']?.header === 'search' ? 'search' : false}
                data={dataTable}
            />
        );
    }
