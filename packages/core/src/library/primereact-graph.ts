...
const BLUE_LT  = solid(0.898, 0.922, 1.000)        // #E5EBff
...
export function buildPrimeReactGraph(): SceneGraph {
  ...
  for (const col of ['ID', 'Name', 'Status', 'Date']) {
    graph.createNode('TEXT', dtHead.id, { name: col, text: col, fontSize: 12, fontWeight: 600, fills: DARK_TXT, layoutGrow: 1 })
  }
  // Data rows
  for (let r = 1; r <= 3; r++) {
    const row = graph.createNode('FRAME', dt.id, {
      ...
      primaryAxisSizing: 'FIXED',
      counterAxisSizing: 'FIXED',
      strokes: [{ color: { r: 0.831, g: 0.859, b: 0.886, a: 1 }, opacity: 1, visible: true, align: 'INSIDE' as const, weight: 1 }],
    })
    for (const val of [`${r}`, `Item ${r}`, 'Active', '2024-01-0' + r]) {
      graph.createNode('TEXT', row.id, { name: val, text: val, fontSize: 13, fills: DARK_TXT, layoutGrow: 1 })
    }
  }
  ...
  graph.createNode('TEXT', card.id, { name: 'title', text: 'Card Title', fontSize: 16, fontWeight: 700, fills: DARK_TXT })
  graph.createNode('TEXT', card.id, { name: 'content', text: 'Card content goes here.', fontSize: 14, fills: GRAY_TXT })
  ...
  graph.createNode('TEXT', dlgHead.id, { name: 'title', text: 'Dialog', fontSize: 16, fontWeight: 700, fills: DARK_TXT })
  graph.createNode('RECTANGLE', dlgHead.id, { name: 'close', width: 20, height: 20, cornerRadius: 4, fills: GRAY_TXT })
  ...
  graph.createNode('FRAME', dlgFoot.id, { name: 'cancel-btn', width: 80, height: 36, cornerRadius: 6, fills: GRAY_BG })
  graph.createNode('FRAME', dlgFoot.id, { name: 'ok-btn', width: 80, height: 36, cornerRadius: 6, fills: BLUE })
  ...
  graph.createNode('TEXT', panelHead.id, { name: 'title', text: 'Panel', fontSize: 14, fontWeight: 700, fills: DARK_TXT })
  ...
  graph.createNode('TEXT', tag.id, { name: 'value', text: 'Tag', fontSize: 11, fontWeight: 600, fills: WHITE })
  ...
  graph.createNode('TEXT', badge.id, { name: 'value', text: '3', fontSize: 11, fontWeight: 700, fills: WHITE })
  ...
  graph.createNode('RECTANGLE', tbLeft.id, { name: 'btn1', width: 60, height: 32, cornerRadius: 4, fills: BLUE })
  graph.createNode('RECTANGLE', tbLeft.id, { name: 'btn2', width: 60, height: 32, cornerRadius: 4, fills: GRAY_BG })
  ...
  for (const [i, label] of [['0', 'Home'], ['1', '/'], ['2', 'Section'], ['3', '/'], ['4', 'Page']] as [string, string][]) {
  ...
  graph.createNode('TEXT', cal.id, { name: 'date', text: 'Select Date', fontSize: 14, fills: GRAY_TXT })
  graph.createNode('RECTANGLE', cal.id, { name: 'icon', width: 16, height: 16, cornerRadius: 2, fills: GRAY_TXT })
  ...
  graph.createNode('ELLIPSE', sl.id, {
    ...
    width: 16, height: 16,
    fills: WHITE,
    strokes: [{ color: { r: 0.247, g: 0.475, b: 0.957, a: 1 }, opacity: 1, visible: true, align: 'CENTER' as const, weight: 2 }],
  })
  ...
  const tab1 = graph.createNode('FRAME', tvTabs.id, {
    ...
    counterAxisAlign: 'CENTER',
    fills: WHITE,
    strokes: [{ color: { r: 0.247, g: 0.475, b: 0.957, a: 1 }, opacity: 1, visible: true, align: 'INSIDE' as const, weight: 2 }],
    primaryAxisSizing: 'FIXED', counterAxisSizing: 'FIXED',
  })
  graph.createNode('TEXT', tab1.id, { name: 'label', text: 'Tab 1', fontSize: 13, fontWeight: 600, fills: BLUE })
  ...
  const msg = graph.createNode('COMPONENT', pageId, {
    ...
    cornerRadius: 6,
    fills: BLUE_LT,
    strokes: [{ color: { r: 0.247, g: 0.475, b: 0.957, a: 0.3 }, opacity: 1, visible: true, align: 'INSIDE' as const, weight: 1 }],
  })
  graph.createNode('ELLIPSE', msg.id, { name: 'icon', width: 20, height: 20, fills: BLUE })
  graph.createNode('TEXT', msg.id, { name: 'text', text: 'This is an info message.', fontSize: 13, fills: DARK_TXT })
