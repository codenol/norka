...
```sh
norka export design.fig                          # PNG (predeterminado)
norka export design.fig -f jpg -s 2 -q 90       # JPG a 2×, calidad 90
norka export design.fig -f webp -s 3             # WEBP a 3×
norka export design.fig -f svg                   # SVG vectorial
```
...
También soporta `--style norka` para el formato JSX nativo (ver [Renderizador JSX](../jsx-renderer)).
...
```sh
norka export -f png    # captura de pantalla del lienzo actual
```
