...
- App identity: pencil icon, Cargo crate norka, macOS Dock "Norka"
...
**Delivered:**
- @norka/core extracted to packages/core/ (zero DOM deps, Bun workspace)
- @norka/cli with headless .fig operations (info, tree, find, export, analyze, node, pages, variables, eval), CanvasKit CPU rasterization, --json output
...
- MCP server (@norka/mcp): stdio + HTTP (Hono + Streamable HTTP with sessions), 87 core tools + 3 file management tools (90 total), runs on Bun and Node.js
...
- VitePress documentation site at norka.dev with i18n (6 languages)
