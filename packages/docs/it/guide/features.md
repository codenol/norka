...
**Copia e incolla con Figma** — seleziona i nodi in Figma, <kbd>⌘</kbd><kbd>C</kbd>, passa a Norka, <kbd>⌘</kbd><kbd>V</kbd>. Riempimenti, bordi, auto-layout, testo, effetti, raggi degli angoli e reti vettoriali vengono preservati. Funziona in entrambe le direzioni.
...
```sh
norka tree design.fig          # Albero dei nodi
norka find design.fig --type TEXT  # Ricerca
norka export design.fig -f png     # Render
norka analyze colors design.fig    # Audit colori
norka analyze clusters design.fig  # Pattern ripetuti
...
```sh
norka tree                     # Documento live
norka export -f png            # Screenshot del canvas
```

Tutti i comandi supportano `--json`. Installazione: `bun add -g @norka/cli`
...
**Web** — disponibile su [app.norka.dev](https://app.norka.dev), installabile come PWA su mobile con interfaccia ottimizzata per il touch.
...
Quando un font non è disponibile localmente, Norka lo scarica automaticamente da Google Fonts. Nessuna installazione manuale necessaria quando si aprono file .fig con font non familiari.
