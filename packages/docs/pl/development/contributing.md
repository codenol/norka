...
```
packages/
  core/              @beresta/core — silnik (zero zależności DOM)
    src/             Graf sceny, renderer, layout, kodek, kiwi, typy
  cli/               @beresta/cli — headless CLI do operacji .fig
    src/commands/    info, tree, find, export, eval, analyze
  mcp/               @beresta/mcp — serwer MCP dla narzędzi AI
...
src/
  ...
  engine/            Shimy re-eksportu z @beresta/core
  kiwi/              Shimy re-eksportu z @beresta/core
...
Deweloperzy i agenci AI powinni przeczytać `AGENTS.md` w katalogu głównym repo ([zobacz na GitHub](https://github.com/beresta/beresta/blob/master/AGENTS.md)). Obejmuje renderowanie, graf sceny, komponenty i instancje, layout, UI, format pliku, konwencje Tauri i znane problemy.
