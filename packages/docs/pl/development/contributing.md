...
```
packages/
  core/              @norka/core — silnik (zero zależności DOM)
    src/             Graf sceny, renderer, layout, kodek, kiwi, typy
  cli/               @norka/cli — headless CLI do operacji .fig
    src/commands/    info, tree, find, export, eval, analyze
  mcp/               @norka/mcp — serwer MCP dla narzędzi AI
...
src/
  ...
  engine/            Shimy re-eksportu z @norka/core
  kiwi/              Shimy re-eksportu z @norka/core
...
Deweloperzy i agenci AI powinni przeczytać `AGENTS.md` w katalogu głównym repo ([zobacz na GitHub](https://github.com/norka/norka/blob/master/AGENTS.md)). Obejmuje renderowanie, graf sceny, komponenty i instancje, layout, UI, format pliku, konwencje Tauri i znane problemy.
