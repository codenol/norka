...
**Copy & paste with Figma** — select nodes in Figma, <kbd>⌘</kbd><kbd>C</kbd>, switch to Norka, <kbd>⌘</kbd><kbd>V</kbd>. Fills, strokes, auto-layout, text, effects, corner radii, and vector networks are preserved. Works both ways.
...
```sh
norka tree design.fig          # Node tree
norka find design.fig --type TEXT  # Search
norka export design.fig -f png     # Render
norka analyze colors design.fig    # Color audit
norka analyze clusters design.fig  # Repeated patterns
...
```sh
norka tree                     # Live document
norka export -f png            # Screenshot canvas
```

All commands support `--json`. Install: `bun add -g @norka/cli`
...
**Web** — runs at [app.norka.dev](https://app.norka.dev), installable as a PWA on mobile with touch-optimized UI.
...
When a font isn't available locally, Norka fetches it from Google Fonts automatically. No manual installation needed when opening .fig files with unfamiliar fonts.
