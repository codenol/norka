...
**Копирование и вставка с Figma** — выделите узлы в Figma, <kbd>⌘</kbd><kbd>C</kbd>, переключитесь на Beresta, <kbd>⌘</kbd><kbd>V</kbd>. Заливки, обводки, авто-макет, текст, эффекты, радиусы скругления и векторные сети сохраняются. Работает в обоих направлениях.
...
```sh
beresta tree design.fig          # Дерево узлов
beresta find design.fig --type TEXT  # Поиск
beresta export design.fig -f png     # Рендеринг
beresta analyze colors design.fig    # Аудит цветов
beresta analyze clusters design.fig  # Повторяющиеся паттерны
...
```sh
beresta tree                     # Текущий документ
beresta export -f png            # Скриншот канваса
```

Все команды поддерживают `--json`. Установка: `bun add -g @beresta/cli`
...
**Веб** — работает на [app.beresta.dev](https://app.beresta.dev), устанавливается как PWA на мобильных устройствах с адаптированным интерфейсом.
...
Когда шрифт недоступен локально, Beresta автоматически загружает его из Google Fonts. Ручная установка не требуется при открытии файлов .fig с незнакомыми шрифтами.
