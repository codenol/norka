...
**Копирование и вставка с Figma** — выделите узлы в Figma, <kbd>⌘</kbd><kbd>C</kbd>, переключитесь на Norka, <kbd>⌘</kbd><kbd>V</kbd>. Заливки, обводки, авто-макет, текст, эффекты, радиусы скругления и векторные сети сохраняются. Работает в обоих направлениях.
...
```sh
norka tree design.fig          # Дерево узлов
norka find design.fig --type TEXT  # Поиск
norka export design.fig -f png     # Рендеринг
norka analyze colors design.fig    # Аудит цветов
norka analyze clusters design.fig  # Повторяющиеся паттерны
...
```sh
norka tree                     # Текущий документ
norka export -f png            # Скриншот канваса
```

Все команды поддерживают `--json`. Установка: `bun add -g @norka/cli`
...
**Веб** — работает на [app.norka.dev](https://app.norka.dev), устанавливается как PWA на мобильных устройствах с адаптированным интерфейсом.
...
Когда шрифт недоступен локально, Norka автоматически загружает его из Google Fonts. Ручная установка не требуется при открытии файлов .fig с незнакомыми шрифтами.
