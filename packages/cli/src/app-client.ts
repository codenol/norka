import { AUTOMATION_HTTP_PORT } from '@beresta/core'
...
export async function getAppToken(): Promise<string> {
  ...
  if (!res || !res.ok) {
    throw new Error(
      `Could not connect to Beresta app on localhost:${AUTOMATION_HTTP_PORT}.\n` +
  ...
  if (data.status !== 'ok' || !data.token) {
    throw new Error(
      'Beresta app is running but no document is open.\n' +
